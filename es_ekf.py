...
#     _t: Timestamp in ms.
#   imu_f: StampedData object with the imu specific force data (given in vehicle frame).在重力以外的加速度
...
################################################################################################
p_est = np.zeros([imu_f.data.shape[0], 3])  # position estimates   xyz_DC
v_est = np.zeros([imu_f.data.shape[0], 3])  # velocity estimates   vx,vy,vz_DC
q_est = np.zeros([imu_f.data.shape[0], 4])  # orientation estimates as quaternions  xyz theta
